...
void swarm_comm_init(void) {
    ...
    /* Set up TX */
    tx_queue = xQueueCreateStatic(sizeof(tx_queue_buf) / sizeof(tx_queue_item_t),
                                  sizeof(tx_queue_item_t),
                                  tx_queue_buf, &tx_queue_static);
    STATIC_MEM_TASK_CREATE(tx_task, swarm_comm_tx_task, "swarm_tx", NULL, 0);
    swarm_selftest_start_if_mapping();
}
...
static void handle_tx_queue_item(tx_queue_item_t *item) {
    ...
    case COMM_TX_QUEUE_ITEM_POSE: {
        ...
        swarm_pose_t pose = { .id = item->pose_id };
    ...
    case COMM_TX_QUEUE_ITEM_CTRL: {
        /* Send the control message (packed 4 bytes) */
...
void swarm_radio_handle_msg(uint8_t src, uint8_t tag, uint8_t *data, size_t len) {
    /* Helpful RX trace (can be silenced later) */
    printf("[RX] src=%u tag=%u len=%u\n",
    ...
    case COMM_MSG_TAG_CTRL: {
        /* Accept variable-length control messages:
           - START/DONE may be 1B type (+ optional pad)
           - POSE may include 2B pose after one pad byte
           We normalize into swarm_comm_ctrl_msg_t before handing off. */
        swarm_comm_ctrl_msg_t msg = {0};
        if (len >= 1) {
            msg.type = data[0];
        }
        if (len >= 4) {
            /* data layout expected: type (0), pad (1), pose (2..3) */
            uint16_t raw;
            memcpy(&raw, &data[2], sizeof(raw));
            msg.pose.raw = raw;
        }
        printf("[MAP] CTRL received type=%u pose=0x%04X\n",
               (unsigned)msg.type, (unsigned)msg.pose.raw);
        swarm_comm_handle_ctrl_msg(src, msg);
        break;
    }
    default:
        DEBUG_PRINT("Unknown tag=%u", (unsigned)tag);
        break;
    }
}
